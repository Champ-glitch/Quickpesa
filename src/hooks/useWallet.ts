...
export const useWallet = () => {
  ...
  const deposit = async (amount: number, _phone: string) => {
    ...
    addTransaction({
      id: `dep_${Date.now()}`, userId: user?.id || '', type: 'deposit',
      amount, status: 'completed', method: 'mpesa',
  ...
  const withdraw = async (amount: number, _phone: string) => {
    ...
    updateBalance(-amount);
    addTransaction({
      id: `wdr_${Date.now()}`, userId: user.id, type: 'withdrawal',
      amount: -amount, status: 'pending', method: 'mpesa',
